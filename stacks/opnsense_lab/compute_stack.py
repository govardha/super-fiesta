    ...
    def __init__(
        ...
        **kwargs,
        ...
        wan_sg = ec2.SecurityGroup(
            self, "WanSG", vpc=vpc, description="OPNsense WAN", allow_all_outbound=True
        )
        ...
        lan_sg = ec2.SecurityGroup(
            self, "LanSG", vpc=vpc, description="OPNsense LAN", allow_all_outbound=True
        )
        lan_sg.add_ingress_rule(
            ec2.Peer.ipv4(vpc.vpc_cidr_block), ec2.Port.all_traffic(), "All from VPC"
        )
        ...
        opnsense_instance = ec2.CfnInstance(
            ...
            network_interfaces=[
                ec2.CfnInstance.NetworkInterfaceProperty(
                    device_index="0", network_interface_id=wan_eni.ref
                ),
            ],
        ...
        instance_sg.add_ingress_rule(
            ec2.Peer.ipv4(vpc.vpc_cidr_block), ec2.Port.all_traffic(), "VPC internal"
        )
        ...
        CfnOutput(
            ...
            "OpnsenseLanEniId",
            value=lan_eni.ref,
            description="Attach this ENI to OPNsense as device index 1 after boot",
        )
        CfnOutput(
            self,
            "OpnsensePublicIP",
        ...
        CfnOutput(
            self,
            "AttachLanCmd",
            value=f"aws ec2 attach-network-interface --instance-id INSTANCE_ID --network-interface-id {lan_eni.ref} --device-index 1 --region {region}",
