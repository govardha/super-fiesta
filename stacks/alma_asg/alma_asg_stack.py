...
class AlmaASGStack(Stack):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if (
            self.infra_config.compute
            and self.infra_config.compute.asg
            and self.infra_config.compute.asg.enable_daily_refresh
        ):
    ...
    def create_launch_template(self):
        ...
        if (
            not self.infra_config.compute
            or not self.infra_config.compute.almalinux_amis
        ):
            raise ValueError(
                "AlmaLinux AMI configuration not found in infrastructure.yaml"
            )
        ...
        if os_version not in self.infra_config.compute.almalinux_amis[region]:
            raise ValueError(
                f"AlmaLinux {os_version} AMI not configured for region {region}"
            )
        ...
        key_name = (
            getattr(self.infra_config.ec2, "key_name", None)
            if hasattr(self.infra_config, "ec2")
            else None
        )
        ...
        self.launch_template = ec2.LaunchTemplate(
            ...
            key_pair=ec2.KeyPair.from_key_pair_name(self, "AlmaASGKeyPair", key_name)
            if key_name
            else None,
    ...
    def create_cloudwatch_alarms(self):
        ...
        # Create custom metric for in-service instances
        in_service_metric = cloudwatch.Metric(
    ...
    def create_scheduled_refresh(self):
        ...
        refresh_lambda = lambda_.Function(
            ...
            environment={
                ...
                "REFRESH_ONLY_ON_DEMAND": "true"
                if asg_config.refresh_only_on_demand
                else "false",
            },
    ...
    def create_outputs(self):
        ...
        CfnOutput(
            self,
            "LaunchTemplateId",
            value=self.launch_template.launch_template_id or "Not available",
